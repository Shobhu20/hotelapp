...
interface RoomRepository : JpaRepository<Room, Int> {
    ...
    @Query("select * from room where not exists (SELECT DISTINCT d.* FROM Room AS d JOIN Booking AS c ON c.room_id = d.id  WHERE d.status LIKE :check AND (c.arrival_time<=:todate AND c.departure_time>=:todate) OR " +
                    "(c.arrival_time<=:fromdate AND c.departure_time>=:fromdate));", nativeQuery = true)
