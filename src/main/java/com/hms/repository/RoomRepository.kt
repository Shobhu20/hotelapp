...
interface RoomRepository : JpaRepository<Room, Int> {
    ...
    @Query("select * from room as r where r.id not in(SELECT DISTINCT d.id FROM Room AS d JOIN Booking AS c ON c.room_id = d.id  WHERE d.status LIKE :check AND (c.from_date<=:todate AND c.to_date>=:todate) OR " +
